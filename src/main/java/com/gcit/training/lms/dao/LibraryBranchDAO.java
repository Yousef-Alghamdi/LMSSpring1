...
public class LibraryBranchDAO extends AbstractDAO implements
	...
	public List<LibraryBranch> readAll(int pageNo, int pageSize)
		...
		return (List<LibraryBranch>) template.query(
				"select * from tbl_library_branch  LIMIT ?,?",
	...
	public List<LibraryBranch> readByName(String searchString, int pageNo, int pageSize) throws SQLException {
		setPageNo(pageNo);
		String qString = "%" + searchString + "%";
		return (List<LibraryBranch>) template.query(
				"select * from tbl_library_branch where branchName like ?  LIMIT ?,?",
